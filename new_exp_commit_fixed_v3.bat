...
if "%SUFFIX%"=="" (
  echo Usage:
  echo   new_exp_commit_fixed_v3.bat short_name
  echo Example:
  echo   new_exp_commit_fixed_v3.bat cat_strict_baseline
  exit /b 1
)
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "  $text = [regex]::Replace($text, '(?m)^#\s*実験名\s*$', ('# 実験名' + $nl + $expId), 1);" ^
  "  $text = [regex]::Replace($text, '(?m)^##\s*タイトル\s*$', ('## タイトル' + $nl + $title), 1);" ^
  ...
  "    if ($lines[$i] -match '^\s*experiment\s*:\s*$') {" ^
  "      $inExperiment = $true;" ^
  "      continue;" ^
  "    }" ^
  "    if ($inExperiment -and $lines[$i] -match '^[^\s]') {" ^
  "      $inExperiment = $false;" ^
  "    }" ^
  "    if ($inExperiment -and $lines[$i] -match '^\s{2}exp_id\s*:') {" ^
  "      $lines[$i] = '  exp_id: ' + $expId;" ^
  "      continue;" ^
  "    }" ^
  "    if ($inExperiment -and $lines[$i] -match '^\s{2}title\s*:') {" ^
  "      $lines[$i] = '  title: ""' + $title + '""';" ^
  "      continue;" ^
  "    }" ^
  "  }" ^
