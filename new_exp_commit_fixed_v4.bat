...
if "%SUFFIX%"=="" (
  echo Usage:
  echo   new_exp_commit_fixed_v4.bat short_name
  echo Example:
  echo   new_exp_commit_fixed_v4.bat cat_strict_baseline
  exit /b 1
)
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "  $text = [regex]::Replace($text, '(?m)^#\s*実験名\s*$[\r\n]+[^\r\n]*', ('# 実験名' + [Environment]::NewLine + $expId), 1);" ^
  "  $text = [regex]::Replace($text, '(?m)^##\s*タイトル\s*$[\r\n]+[^\r\n]*', ('## タイトル' + [Environment]::NewLine + $title), 1);" ^
  ...
  "  $pattern = '(?ms)^experiment:\s*[\r\n]+(?<body>(?:^[ ]{2}.*(?:\r?\n|$))+)';" ^
  "  $text = [regex]::Replace($text, $pattern, {" ^
  "    param($m)" ^
  "    $body = $m.Groups['body'].Value;" ^
  "    $body = [regex]::Replace($body, '(?m)^[ ]{2}exp_id\s*:.*$', ('  exp_id: ' + $expId), 1);" ^
  "    $body = [regex]::Replace($body, '(?m)^[ ]{2}title\s*:.*$', ('  title: ""' + $title + '""'), 1);" ^
  "    return 'experiment:' + [Environment]::NewLine + $body" ^
  "  }, 1);" ^
